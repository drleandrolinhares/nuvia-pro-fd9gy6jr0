...
import { FETLogs } from '@/components/fet/fet-logs'
...
export default function FET() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] md:h-full bg-slate-950 overflow-hidden rounded-xl border border-slate-800 shadow-2xl -mx-2 sm:-mx-4 md:-mx-8 lg:-mx-16 w-[calc(100%+1rem)] sm:w-[calc(100%+2rem)] md:w-[calc(100%+4rem)] lg:w-[calc(100%+8rem)] max-w-none">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 border-b border-slate-800 bg-slate-900/30 shrink-0">
        ...
        >
          <TabsList className="bg-slate-900/50 border border-slate-800 h-9">
            <TabsTrigger
              value="ativos"
              className="data-[state=active]:bg-amber-500 data-[state=active]:text-slate-950 text-xs font-bold"
            >
              Tratamentos Ativos
            </TabsTrigger>
            <TabsTrigger
      ...
      <div className="flex flex-col md:flex-row flex-1 min-h-0 w-full max-w-full">
        {activeTab !== 'logs' ? (
          <div className="flex flex-col md:flex-row flex-1 gap-3 p-2 md:p-3 min-h-0 w-full max-w-full">
            <div className="w-full md:w-64 lg:w-72 h-full flex flex-col min-h-0 shrink-0">
              <FETPatientList
                status={activeTab === 'ativos' ? 'ativo' : 'finalizado'}
            ...
            <div className="w-full flex-1 h-full flex flex-col min-h-0">
              {selectedPatientId ? (
                <FETPatientDetail
                  ...
                  onStatusChange={() => setSelectedPatientId(null)}
                />
              ) : (
                <div className="flex flex-col flex-1 items-center justify-center border border-slate-800 border-dashed rounded-lg bg-slate-900/30 text-center p-8">
                  ...
                  <p className="text-slate-500 max-w-sm">
                    Selecione um paciente na lista ao lado ou crie um novo para visualizar e
                    gerenciar sua Ficha de Evolução de Tratamento.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex-1 p-2 md:p-3 min-h-0 w-full max-w-full">
            <FETLogs />
          </div>
        )}
      </div>
