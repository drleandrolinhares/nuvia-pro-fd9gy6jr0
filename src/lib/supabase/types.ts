...
//   $function$
//
// FUNCTION get_funil_competencia_metrics(text)
//   CREATE OR REPLACE FUNCTION public.get_funil_competencia_metrics(p_mes_referencia text)
//    RETURNS TABLE(origem_id uuid, origem_nome text, qtd_oportunidades bigint, valor_oportunidades numeric, qtd_vendas bigint, valor_vendas numeric)
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
//   BEGIN
//       RETURN QUERY
//       WITH origens_validas AS (
//           SELECT id, nome, ordem
//           FROM public.funil_origens
//           WHERE ativo = true AND nome ILIKE '%recorrente%' = false
//       ),
//       oportunidades AS (
//           SELECT
//               a.origem_id,
...
//           INNER JOIN public.pacientes p ON p.id = a.paciente_id
//           WHERE a.origem_id IN (SELECT id FROM origens_validas)
//             AND TRIM(COALESCE(p.nome, '')) != ''
//             AND to_char(COALESCE(a.data_avaliacao, (a.criado_em AT TIME ZONE 'America/Sao_Paulo')::date), 'YYYY-MM') = p_mes_referencia
...
//           WHERE v.origem_id IN (SELECT id FROM origens_validas)
...
// Table: vendas_confirmadas
//   CREATE UNIQUE INDEX idx_vendas_confirmadas_unique_venda ON public.vendas_confirmadas USING btree (lower(TRIM(BOTH FROM paciente_nome)), data_fechamento, COALESCE(origem_id, '00000000-0000-0000-0000-000000000000'::uuid))
//   CREATE INDEX vendas_confirmadas_oportunidade_id_idx ON public.vendas_confirmadas USING btree (oportunidade_id)
// Table: vendas_diarias
//   CREATE UNIQUE INDEX idx_vendas_diarias_unique_venda ON public.vendas_diarias USING btree (lower(TRIM(BOTH FROM paciente_nome)), data_venda, COALESCE(origem_id, '00000000-0000-0000-0000-000000000000'::uuid))
